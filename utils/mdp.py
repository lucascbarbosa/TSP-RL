...
def build_mdp_model(transition_filename: str):
    ...
    return build_mdp_model_from_data(current_states, actions, rewards, next_states, n_states, n_actions)
...
def build_mdp_model_from_data(current_states, actions, rewards, next_states, n_states, n_actions):
    """Build MDP model from transitions."""
    ...
    transition_matrix = torch.where(
        visits_mask_expanded, transition_matrix / visit_count_expanded, torch.zeros_like(transition_matrix)
    )
    reward_matrix = torch.where(visits_mask, reward_matrix / visit_count, torch.zeros_like(reward_matrix))
