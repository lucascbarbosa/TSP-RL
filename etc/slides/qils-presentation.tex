...
% --- Metadados ---
\title[\sutil{Q-ILS}]{Q-ILS: Integrando Q-Learning \\ e Iterated Local Search para o TSP}
...
\begin{frame}{Objetivos}
  \footnotesize
  \begin{enumerate}
    \item Propor uma abordagem híbrida \destaque{\emph{Q-ILS}} que integra Q-Learning ao framework do Iterated Local Search.
    \item \destaque{Aprender}, a partir de execuções em instâncias de treino, qual combinação (construtivo + busca local) tende a melhorar soluções.
    \item \destaque{Avaliar} se a política aprendida generaliza para instâncias não vistas.
  \end{enumerate}
  \vspace{4mm}
  \begin{calloutbox}
    {\scriptsize
    Foco em reduzir o \emph{gap} (em relação ao ótimo conhecido) de forma automática, sem ajuste manual de parâmetros.
    }
  \end{calloutbox}
\end{frame}
...
\begin{frame}{Componentes do Q-ILS}
  \footnotesize
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item \destaque{3-opt}: troca 3 arestas; mais lento
        \item \destaque{Lin-Kernighan}: busca variável; mais potente
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{Perturbações:}
      ...
      \begin{alertbox}
        {\scriptsize
        \textbf{9 combinações possíveis:} \\
        3 construtivos $\times$ 3 buscas locais
        }
      \end{alertbox}
    \end{column}
  \end{columns}
...
\begin{frame}{Modelagem como MDP}
  ...
  \begin{itemize}
    ...
    \item \destaque{Ação $a$}: (construtivo, busca local)
...
\begin{frame}{Discretização de Estados}
  ...
  \begin{tabular}{cccc}
    ...
    $\text{gap} > 10$ & 3 & 0 & Ruim \\
    $\text{gap} < 0$ & 4 & 100 & Melhor que ótimo \\
    \bottomrule
  \end{tabular}
  \vspace{4mm}

  \[
    \text{gap} = \frac{\text{custo}_{\text{atual}} - \text{custo}_{\text{ótimo}}}{\text{custo}_{\text{ótimo}}} \times 100\%
  \]
\end{frame}
...
\begin{frame}{Espaço de Ações}
  ...
  \begin{tabular}{cll}
    \toprule
    \textbf{Ação} & \textbf{Construtivo} & \textbf{Busca Local} \\
    \midrule
    0 & Random & 2-opt \\
    1 & Random & 3-opt \\
    2 & Nearest Neighbor & 2-opt \\
    3 & Nearest Neighbor & 3-opt \\
    4 & Cheapest Insertion & 2-opt \\
    5 & Cheapest Insertion & 3-opt \\
    6 & Random & Lin-Kernighan \\
    7 & Nearest Neighbor & Lin-Kernighan \\
    8 & Cheapest Insertion & Lin-Kernighan \\
    \bottomrule
  \end{tabular}
  \vspace{3mm}
...
\begin{frame}{Loop Principal do Q-ILS}
  \vspace{-3mm}
  \footnotesize
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{enumerate}
        \item Gerar solução inicial
        \item Aplicar busca local (3-opt)
        \item Loop:
          \begin{itemize}
            \item Perturbar solução
            \item Calcular estado $s$ (gap)
            \item Consultar Q-table: $a = \arg\max Q(s,a)$
            \item Executar ação $a$
            \item Atualizar melhor solução
          \end{itemize}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{alertbox}
        ...
        $\bullet$ Requer ótimo conhecido no treino \\
...
\begin{frame}{Discussão}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      \textbf{Desafios:}
      \begin{itemize}
        \item Dependência do ótimo conhecido
...
\begin{frame}{Conclusões Esperadas}
  \footnotesize
  \begin{itemize}
    \item \destaque{Q-ILS} é viável para integrar RL a meta-heurísticas para o TSP.
